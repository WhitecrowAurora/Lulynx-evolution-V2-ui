...
const animaConceptEditNetworkFields = [
  ...
  { key: 'bypass_mode', type: 'boolean', label: 'Bypass Mode', title: 'bypass_mode', desc: 'Bypass Mode', defaultValue: false, visibleWhen: (c) => c.lora_type === 'lora' && !c.dora_wd },
  { key: 'adapter_init_strategy', type: 'select', label: 'LoRA 初始化策略', title: 'adapter_init_strategy', desc: '统一初始化入口：默认 LoRA / PiSSA /', defaultValue: 'default', options: ADAPTER_INIT_STRATEGY_OPTIONS, visibleWhen: (c) => c.lora_type === 'lora' && !c.dora_wd },
  ...
  { key: 'pissa_init', type: 'boolean', label: '启用 PiSSA 初始化', title: 'pissa_init', desc: '产品入口：开启后映射', defaultValue: false, visibleWhen: (c) => c.lora_type === 'lora' && !c.dora_wd },
  { key: 'pissa_enabled', type: 'boolean', label: 'PiSSA 后端 master', title: 'pissa_enabled', desc: '后端 injector 主开关', defaultValue: false, visibleWhen: (c) => c.lora_type === 'lora' && !c.dora_wd },
