...
function normalizeAdapterFamily(value) {
...
export function resolveAdapterFamily(config = {}) {
  ...
  if (network === 'networks.oft') return 'diag-oft';
  if (network.includes('lycoris')) return normalizeAdapterFamily(algo || 'loha');
  if (config.dora_enabled === true || config.use_dora === true || config.dora_wd === true) return 'dora';
...
export function applyAdapterFamilyCapabilities(payload = {}) {
  ...
  for (const [rawFamily, capability] of Object.entries(source)) {
    ...
    next[normalizeAdapterFamily(rawFamily)] = { ...capability };
  }
  if (!Object.keys(next).length) return false;
  adapterFamilyCapabilities = { ...FALLBACK_ADAPTER_FAMILIES, ...next };
  return true;
}
...
const ADAPTER_ENTITY_KEYS = Object.freeze(ADAPTER_ENTITY_PRIORITY.map((e) => e.key));

/** DoRA/AdaLoRA 只挂在默认 LoRALinear 上；换实体后应关闭。 */
...
/**
 * 解析当前配置下的适配器实体赢家。
 * @returns {{ id: string, key: string|null, label: string, source: 'lora_type'|'enabled_flag'|'default' }}
 */
...
export function normalizeAdapterEntityMutex(payload = {}) {
  ...
  if (winner.id !== 'lora') {
    ...
    // dora_wd 是 Anima 网络区另一套 DoRA 入口
    if (payload.dora_wd) payload.dora_wd = false;
  }
